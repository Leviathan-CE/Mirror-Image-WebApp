...
describe("battlefield / stockpile seating preserves faceDown", () => {
  ...
  it("library → battlefield seats face down even if the flag was unset", () => {
    const next = moveToBattlefield(
      [card({ instanceId: "a", zone: PLAY_ZONE.library })],
      "a",
      10,
      20
    )
  ...
  it("hand → battlefield keeps face up", () => {
    const next = moveToBattlefield(
      [card({ instanceId: "a", zone: PLAY_ZONE.hand, faceDown: false })],
      "a",
      10,
      20
    )
    expect(next[0]?.faceDown).toBe(false)
  })

  it("face-down battlefield → hand / trash / dismantled reveals", () => {
    const faceDownField = card({
      instanceId: "a",
      zone: PLAY_ZONE.battlefield,
      faceDown: true,
      x: 1,
    ...
    expect(moveToHand([faceDownField], "a")[0]?.faceDown).toBe(false)
    expect(moveToTrashyard([faceDownField], "a")[0]?.faceDown).toBe(false)
    expect(moveToDismantled([faceDownField], "a")[0]?.faceDown).toBe(false)
  })
})
