...
/**
 * Face-down persists across zones except when entering a public pile:
 * hand / trashyard / dismantled always seat face up.
 * Leaving the library into play (battlefield / stockpile / pilot) seats
 * face down — the deck is opaque until those reveal zones.
 */
function faceDownOnEnter(
  card: PlayingCardInstance,
  target: PlayZone
): boolean | undefined {
  if (
    target === PLAY_ZONE.hand ||
    target === PLAY_ZONE.trashyard ||
    target === PLAY_ZONE.dismantled
  ) {
    return false
  }
  if (
    card.zone === PLAY_ZONE.library &&
    (target === PLAY_ZONE.battlefield ||
      target === PLAY_ZONE.stockpile ||
      target === PLAY_ZONE.pilot)
  ) {
    return true
  }
...
function seatCard(
  ...
  const expended =
    spec.expended === "inPlay"
      ...
      : false
  const faceDown = faceDownOnEnter(card, spec.zone)
  ...
  for (const c of cards) {
    if (c.instanceId === instanceId) {
      next.push({
        ...
        y: undefined,
        expended: false,
        faceDown: faceDownOnEnter(c, PLAY_ZONE.pilot),
        selected: false,
      })
      continue
    }
    if (c.zone === "pilot" && c.owner === seated.owner) {
      ...
      next.push({
        ...
        y: undefined,
        expended: false,
        faceDown: faceDownOnEnter(c, PLAY_ZONE.hand),
        selected: false,
      })
      continue
    }
