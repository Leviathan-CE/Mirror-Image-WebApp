...
import type { PlayFx, PlayFxZone } from "@/components/Playtester/net/playNet.logic";
import {
  FACE,
  displayToWorld,
...
export function useDrawAnimations({
  ...
  function onDeckTopRelease(clientX: number, clientY: number) {
    ...
    if (pointInRect(clientX, clientY, zoneRefs.hand.current)) {
      ...
      // Pointer is display-logical; session stores world. Centre the card on
      // the cursor so the settled tile matches the fly endpoint (not a jump).
    ...
    const stayFaceDown =
      landZone === PLAY_ZONE.battlefield || landZone === PLAY_ZONE.pilot;
    const flyingCard = {
      ...drawn,
    ...
    if (landZone === PLAY_ZONE.battlefield) {
      dispatch({ t: "fr", i: drawn.instanceId });
    }
    pushFlipAnim({
      card: flyingCard,
