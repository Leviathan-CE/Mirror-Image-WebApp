...
SELECT 'admin', 'admin@localhost', '$2b$12$wvLPlK2HuKxCBFITEl/KbO.Scr543iDjETFgr7GzLLuISeyNmjS7i', 'admin'
WHERE NOT EXISTS (
    SELECT 1 FROM users WHERE lower(email) = 'admin@localhost'
);

INSERT INTO users (user_name, email, password, role)
SELECT 'distributor', 'store@localhost', '$2b$12$8H6imr2tcAWRa55vcFbPCe0FyJ5l/XcnLNVfF9jOhf.q0T7CK.A7m', 'distributor'
WHERE NOT EXISTS (
    SELECT 1 FROM users WHERE lower(email) = 'store@localhost'
);

INSERT INTO users (user_name, email, password, role)
SELECT 'user', 'user@localhost', '$2b$12$1Op78lbFAYXl3JcDr4uJSOFNg16BUYHP21sGmQX2qlZ7Ciu182zf6', 'user'
WHERE NOT EXISTS (
